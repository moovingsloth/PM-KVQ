...
This loads the 14B weights once for calibration (max-keys, scale search,
sensitivity, allocation), then BF16 and PM-KVQ evaluation of problem 1 from
each AIME2025 subset (indices 0 and 15), with one response per problem. It
creates a new timestamped output directory and records each command, exit code,
elapsed time and response. Failure stops the pipeline; inspect the named stage
...
On this `spark` branch, BF16 (`original`) responses are generated in small
batches (cap 4) so one decode step rereads the 28 GiB weights once for several
samples. PM-KVQ stays **one sequence at a time**: progressive `n_bits`, sink,
...
This branch applies host-side speedups that keep the paper 1,024 MiB budget:
...
`main` keeps the original one-sequence, one-stage-one-process loop so the
in-flight day run is unchanged. Weight-only quantization, vLLM, and raising
the KV budget are out of scope: they change the experiment, not just the host
...
This `spark` branch cuts repeated 14B loads and batches the four original
responses; PM-KVQ is still sequential, so wall time is still dominated by long
CoT. This is not a paper score.
...
output limit, and seeds 42–57 for each problem. On this branch original
responses are generated in batches of 4 (960 responses total still). PM-KVQ
stays sequential. PM-KVQ uses the 1,024 MiB per-request budget and mixed
`{2,4}` settings; `fake` measures numerical accuracy without physical KV
compression. Calibration artifacts are regenerated at 512 samples, with one
model load. Pass `--no-wandb` to skip logging.
